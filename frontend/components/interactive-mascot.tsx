...
export const InteractiveMascot: React.FC<MascotProps> = ({
  ...
  const sizeConfig = {
    sm: { size: 48, eyeSize: 'w-3 h-3', pupilSize: 'w-1.5 h-1.5', eyeTop: 'top-4', eyeSpacing: 'space-x-1' },
    md: { size: 64, eyeSize: 'w-4 h-4', pupilSize: 'w-2 h-2', eyeTop: 'top-5', eyeSpacing: 'space-x-1.5' },
    lg: { size: 80, eyeSize: 'w-5 h-5', pupilSize: 'w-2.5 h-2.5', eyeTop: 'top-6', eyeSpacing: 'space-x-2' }
  }
  ...
  return (
    ...
    >
      ...
      <div className={cn('absolute left-1/2 transform -translate-x-1/2 flex', config.eyeTop, config.eyeSpacing)}>
        {/* Left Eye - Round shape */}
        <div className={cn('relative bg-white rounded-full border-2 border-gray-800', config.eyeSize)}>
          <div
            className={cn('absolute bg-gray-900 rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2', config.pupilSize)}
            style={{
