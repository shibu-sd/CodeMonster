...
# still allow override for execution
JAVA_OPTS=${JAVA_OPTS:-"-Xmx32m -Xms8m -XX:+UseSerialGC"}

if [ ! -f "/workspace/input.txt" ]; then
    javac -cp /workspace /workspace/Solution.java Runner.java 2>/tmp/compile_error.txt
    ...
    ...
    # Run the test runner (which will report compilation success)
    java $JAVA_OPTS -cp /workspace:/ Runner
else
    # Execution test - compile and run user's standalone program
    ...
    # Run user program with input redirection and capture output
    start_time=$(date +%s%3N)
    output=$(java $JAVA_OPTS -cp /workspace Solution < /workspace/input.txt 2>&1)
    exit_code=$?
    end_time=$(date +%s%3N)
